...
export class UploadsService {
    ...
    async uploadSyllabus(courseId: string, file: Express.Multer.File, userId: string): Promise<FileRecord> {
        ...
        if (user.role === UserRole.LECTURER && course.lecturer.id !== userId) {
            throw new ForbiddenException('You can only upload syllabi for your own courses');
        }
    ...
    async submitAssignment(assignmentId: string, files: Express.Multer.File[], userId: string) {
    ...
    async getAssignmentSubmissions(assignmentId: string, userId: string) {
        ...
        // Check permissions
        if (user.role === UserRole.LECTURER && assignment.course.lecturer.id !== userId) {
        ...
        const submissionDetails = await Promise.all(
            submissions.map(async (submission) => {
                ...
                const files = submission.files.map(fileId => {
    ...
    async getFileInfo(fileId: string, userId: string): Promise<FileRecord> {
        ...
        if (fileRecord.uploadedBy !== userId && user.role !== UserRole.ADMIN) {
            ...
            // For assignments, only the uploader, course lecturer, or admin can access
                ...
                if (user.role === UserRole.STUDENT && fileRecord.uploadedBy !== userId) {
                    throw new ForbiddenException('You do not have permission to access this file');
                }
            }
        }
