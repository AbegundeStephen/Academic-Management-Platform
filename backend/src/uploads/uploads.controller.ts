...
const createMulterOptions = (destination: string) => ({
  ...
  fileFilter: (req, file, callback) => {
    ...
    if (allowedMimes.includes(file.mimetype)) {
      ...
      callback(
        new HttpException('File type not supported', HttpStatus.BAD_REQUEST),
        false,
      );
    }
  },
...
export class UploadsController {
  ...
  @UseInterceptors(
    FilesInterceptor('files', 5, createMulterOptions('assignments')),
  )
  ...
  async getSyllabus(@Param('courseId') courseId: string, @Res() res: Response) {
    const syllabusInfo = await this.uploadsService.getSyllabus(courseId);
    const filePath = join(
      process.cwd(),
      'uploads',
      'syllabi',
      syllabusInfo.filename,
    );
    ...
    res.setHeader(
      'Content-Disposition',
      `inline; filename="${syllabusInfo.originalName}"`,
    );
