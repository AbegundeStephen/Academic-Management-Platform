...
export class AiService {
  ...
  async getCourseRecommendations(
    ...
    if (user.role !== UserRole.STUDENT) {
      throw new ForbiddenException(
        'Only students can get course recommendations',
      );
    }
  ...
  async generateAssignmentFeedback(
    ...
    if (!user || user.role !== UserRole.LECTURER) {
      throw new ForbiddenException(
        'Only lecturers can generate assignment feedback',
      );
    }
    ...
    if (assignment.course.lecturer.id !== userId) {
      throw new ForbiddenException(
        'You can only provide feedback for your own assignments',
      );
    }
  ...
  async generateStudentAnalytics(userId: string, studentId: string) {
    ...
    if (
      ...
      throw new ForbiddenException(
        'Only lecturers and admins can access student analytics',
      );
    }
    ...
    return availableCourses
      .map((course) => {
        ...
        if (preferences.preferredDifficulty) {
          if (
            course.credits <= 3 &&
            preferences.preferredDifficulty === 'beginner'
          ) {
            matchScore += 15;
            ...
            course.credits >= 5 &&
            preferences.preferredDifficulty === 'advanced'
          ) {
            matchScore += 15;
        ...
        if (preferences.academicBackground) {
          ...
          if (
            backgroundKeywords.includes(
              preferences.academicBackground.toLowerCase(),
            )
          ) {
            matchScore += 25;
            reasons.push(
              `Relevant to your ${preferences.academicBackground} background`,
            );
          }
        }
        ...
        const hasPrerequisites = enrolledCourses.some((enrolled) =>
          course.description
            .toLowerCase()
            .includes(enrolled.courseCode.toLowerCase()),
        );
        if (hasPrerequisites) {
          matchScore += 10;
  ...
  private createStudyPlan(course: Course) {
    ...
    return {
      weeklySchedule,
      totalWeeks,
      estimatedStudyHours: weeklySchedule.reduce(
        (total, week) => total + week.studyHours,
        0,
      ),
    };
  }
  ...
  private generateFeedback(
    ...
    if (!submissionContent.includes('example')) {
      improvements.push(
        'Include more concrete examples to support your arguments',
      );
    }
  ...
  private calculateStudentAnalytics(student: User) {
    ...
    const overallGrade =
      (attendanceRate + submissionRate + participationScore) / 3;
