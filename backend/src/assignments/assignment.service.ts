...
export class AssignmentsService {
    ...
    async create(createAssignmentDto: CreateAssignmentDto, lecturerId: string): Promise<Assignment> {
    ...
    async findAll(courseId?: string, lecturerId?: string, user?: User): Promise<Assignment[]> {
        ...
        if (user && user.role === UserRole.STUDENT) {
            query.leftJoin('course.enrollments', 'enrollment')
    ...
    async update(id: string, updateAssignmentDto: UpdateAssignmentDto, lecturerId: string): Promise<Assignment> {
        ...
        const lecturer = await this.userRepository.findOne({ where: { id: lecturerId } });
        if (lecturer.role !== UserRole.ADMIN && assignment.createdBy.id !== lecturerId) {
    ...
    async gradeSubmission(assignmentId: string, submissionId: string, gradeDto: GradeAssignmentDto, lecturerId: string): Promise<AssignmentSubmission> {
        const assignment = await this.assignmentRepository.findOne({
            where: { id: assignmentId },
            relations: ['createdBy'],
        });
        ...
        if (gradeDto.points > assignment.maxPoints) {
            throw new BadRequestException(`Points cannot exceed maximum points (${assignment.maxPoints})`);
        }
    ...
    async getSubmissions(assignmentId: string, lecturerId: string): Promise<AssignmentSubmission[]> {
        const assignment = await this.assignmentRepository.findOne({
            where: { id: assignmentId },
            relations: ['createdBy'],
        });
    ...
    async getStudentSubmission(assignmentId: string, studentId: string): Promise<AssignmentSubmission> {
        const assignment = await this.assignmentRepository.findOne({
            where: { id: assignmentId },
            relations: ['course', 'course.enrollments', 'course.enrollments.student'],
        });
