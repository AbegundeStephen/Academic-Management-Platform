...
export class EnrollmentsController {
    ...
    @ApiResponse({ status: 400, description: 'Bad request - Course full or student already enrolled' })
    ...
    @ApiResponse({ status: 403, description: 'Forbidden - Can only view own enrollments' })
    @ApiResponse({ status: 404, description: 'Student not found' })
    findByStudent(@Param('studentId', ParseUUIDPipe) studentId: string, @CurrentUser() user: User) {
        return this.enrollmentsService.findByStudent(studentId, user);
    }

    @Get('course/:courseId')
    @Roles(UserRole.ADMIN, UserRole.LECTURER)
    @ApiOperation({ summary: 'Get enrollments for a specific course (Admin/Lecturer only)' })
    ...
    findByCourse(@Param('courseId', ParseUUIDPipe) courseId: string, @CurrentUser() user: User) {
    ...
    @ApiResponse({ status: 403, description: 'Forbidden - Cannot drop this enrollment' })
    @ApiResponse({ status: 404, description: 'Enrollment not found' })
