...
export class EnrollmentsService {
    ...
    if (user.role === UserRole.STUDENT) {
      ...
      if (!studentId) {
        throw new BadRequestException(
          'Student ID is required for admin/lecturer enrollment',
        );
      }
  ...
  async findAll(user: User): Promise<Enrollment[]> {
    ...
    if (user.role === UserRole.STUDENT) {
      queryBuilder.where('enrollment.studentId = :studentId', {
        studentId: user.id,
      });
    } else if (user.role === UserRole.LECTURER) {
      queryBuilder.where('course.lecturerId = :lecturerId', {
        lecturerId: user.id,
      });
    }
  ...
  async findOne(id: string, user: User): Promise<Enrollment> {
    ...
    if (
      user.role === UserRole.LECTURER &&
      enrollment.course.lecturerId !== user.id
    ) {
      throw new ForbiddenException(
        'You can only view enrollments for your courses',
      );
    }
  ...
  async updateStatus(
    ...
    if (
      ...
      throw new ForbiddenException(
        'Only admin or course lecturer can update enrollment status',
      );
    }
  ...
  async updateGrade(
    ...
    if (
      ...
      throw new ForbiddenException(
        'Only admin or course lecturer can update grades',
      );
    }
  ...
  async remove(id: string, user: User): Promise<void> {
    ...
    if (
      ...
      throw new ForbiddenException(
        'You can only remove enrollments for your courses',
      );
    }
  ...
  async getEnrollmentStats(user: User): Promise<any> {
    ...
    const completed = await queryBuilder
      ...
      .getCount();
    const dropped = await queryBuilder
      .clone()
      .andWhere('enrollment.status = :status', {
